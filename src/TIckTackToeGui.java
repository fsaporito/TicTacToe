...
public class TIckTackToeGui extends JFrame implements ActionListener {

	private static final long serialVersionUID = 1L;

	private TIckTackToe tris;
	...
	private TickTackToeGuiWin guiWin;
		...
		if (!this.win && !this.draw) {
			...
			try {
				...
				if (this.turn.equals(p1)) {
					...
					if (!this.tris.getStringAtPos(i,j).equals(symbol2)) {
						...
						if (this.tris.checkVincitor(symbol1)) {
							...
							this.guiWin = new TickTackToeGuiWin(this.message.getText());
				...
				if (!this.win && this.tris.checkDraw()) {
					...
					this.message.setText(TIckTackToeGui.stringDraw);
	...
	public static void main(String[] args) {
		...
		TIckTackToeGuiPlayer guiPlayer = new TIckTackToeGuiPlayer();
		...
		TIckTackToeGui guiTris = new TIckTackToeGui();
